...
{
  // as inline functions are used and the interface cannot be changed
  // nullp() and listp() are not used in the line below
  if (my_cdr != nil && !(my_cdr->is_cons())) {
    throw std::runtime_error("cdr can only store ConsCell or null");
  }
